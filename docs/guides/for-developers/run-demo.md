...
Sample data lives under each realm's `raw/` directory:

```text
vault/ACME_CORP/SECURITY_CONSOLIDATION/raw/   # meetings + daily ops
vault/GLOBEX/OBSERVABILITY_RENEWAL/raw/        # meetings + daily ops
vault/INITECH/SEARCH_EVALUATION/raw/           # meetings
```

Each realm also has a `realm_profile.yaml` and each node has a `node_profile.yaml` with engagement metadata.
...
Agents generate artifacts split by audience:
...
```text
vault/ACME_CORP/SECURITY_CONSOLIDATION/raw/meetings/external/
...
```bash
cat vault/ACME_CORP/SECURITY_CONSOLIDATION/internal-infohub/risks/risk_register.yaml
```
...
```bash
cat vault/ACME_CORP/SECURITY_CONSOLIDATION/external-infohub/context/account_profile.md
```
...
```text
vault/GLOBEX/OBSERVABILITY_RENEWAL/raw/meetings/external/2026-01-12-qbr-review.md
```
