...
[image: Knowledge Pipeline - three-stage flow from collection through curation to delivery]
...
deal_sources:
  discovery_outputs:
    location: "{realm}/{node}/external-infohub/context/"
    artifacts:
  ...
  poc_outputs:
    location: "{realm}/{node}/external-infohub/journey/"
    artifacts:
      - type: "poc_results"
  ...
  outcome_data:
    location: "{realm}/{node}/external-infohub/value/"
    artifacts:
...
[image: Knowledge Delivery Modes - pull, push, and proactive delivery paths to different user types]
...
best_practice:
  ...
  relationships:
    ...
    source_artifacts:
      - "ACME/wins/external-infohub/value/security_eval_2026-01.md"
      - "BIGCO/poc/external-infohub/journey/detection_demo_success.md"
...
warning:
  ...
  source_artifacts:
    - "FAILCO/poc/external-infohub/journey/post_mortem.md"
```
