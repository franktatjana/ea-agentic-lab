...
- `infohub/{realm}/{node}/governance/alerts/`
- `notification_queue`
...
- `infohub/{realm}/{node}/architecture/`
...
outputs:
  - output_id: "risk_entry"
    type: "artifact"
    format: "yaml"
    destination: "infohub/{realm}/{node}/risks/risk_register.yaml"
```
...
outputs:
  - output_id: "decision_record"
    type: "decision"
    format: "yaml"
    destination: "infohub/{realm}/{node}/decisions/decision_log.yaml"
```
...
Following our [tool design principles](../design/tool-design-principles.md), all error handling must be **actionable** - providing specific guidance rather than opaque error codes.
